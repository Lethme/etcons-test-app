...
declare interface Array<T> {
	...
	/**
	 * Return the minimal available number from number array
	 *
	 * @description [1, 2, 3].minAvailableNumber() // 4
	 * @description [1, 3, 4, 10].minAvailableNumber() // 2
	 */
	minAvailableNumber(): T extends number ? T : never;
}
...
Object.defineProperty(Array.prototype, 'minAvailableNumber', {
	value: function <T extends number>(this: Array<T>): T extends number ? T : never {
		const arr = this.distinct().sort();

		if (arr.length) {
			for (let i = 0; i < arr.length - 1; i++) {
				if (arr.at(i + 1) - arr.at(i) > 1) {
					return arr.at(i) + 1 as any;
				}
			}

			return arr.at(-1) + 1 as any;
		}
...
declare interface Number {
	/**
	 * Returns true if number belongs to provided interval.
	 * @param from
	 * @param to
	 * @param options
	 */
	in(from: number, to: number, options?: { inclusiveFrom?: boolean, inclusiveTo?: boolean }): boolean;
}

Object.defineProperty(Number.prototype, 'in', {
	value: function (this: number, from: number, to: number, options?: { inclusiveFrom: boolean, inclusiveTo: boolean }) {
		if (!options) {
			options = {
				inclusiveFrom: true,
				inclusiveTo: true,
		...
		const fromState = options && options.inclusiveFrom ? (this >= from) : (this > from);
		const toState = options && options.inclusiveTo ? (this <= to) : (this < to);
